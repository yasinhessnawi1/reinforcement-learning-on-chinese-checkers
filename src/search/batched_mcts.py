...
class BatchedAlphaZeroMCTS:
    ...
    # --- SCM hooks: same structure as AlphaZeroMCTS so SCM collection can use
    # batched MCTS on GPU for ~5-10x speedup over the non-batched path. ---
    ...
    def _subtree_depth(node: MCTSNode, current_depth: int = 1) -> int:
        ...
        if not visited:
            return current_depth
        return max(
            BatchedAlphaZeroMCTS._subtree_depth(c, current_depth + 1)
            for c in visited
        )
    ...
    def _extract_stats_from_root(
        ...
        """Extract SearchStats from a completed MCTS root.

        Mirrors AlphaZeroMCTS._extract_stats_from_root so the SCM training
        loop can consume root statistics from either engine.
        """
    ...
    def get_action_probs_with_stats(
        ...
        """Run batched MCTS and return (action_probs, search_stats) for SCM."""
        root = self.run(env)
        num_actions = env.action_space.n
        mask = env.action_masks()
        probs = self._visits_to_probs(root, num_actions, mask, temperature)
